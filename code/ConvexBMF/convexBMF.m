...
function [Z,V,c] = convexBMF(name, Lambda, Tau,eta_rate)
load([name '/R.mat']);
load([name '/Z0.mat']);
load([name '/W0.mat']);
QQ=imread([name '/R1.png']);
TOL = 1e-3;
T = 300;
T2 = 1;
%T_A = 100;
...
for t = 1:T
	...
	if length(c)~=0
		Vc = diag(sqrt(c))*V;
		R1 = Z*Vc;
		save([name '_guess/Z.mat'],'Z');
		save([name '_guess/Vc.mat'],'Vc');
	else
		Vc = V; 
		R1 = Zc*V;
		Z = Zc;
	...
	['t=' num2str(t) ', d-obj=' num2str(dobj) ', p-obj=' num2str(pobj) ', nnz(c)=' num2str(nnz(c)) ', error=' num2str(norm(R-R1,'fro'))]
	if(7 ~= exist([name '_guess'],'dir')) 
		mkdir( [name '_guess']);
	end
	...
	for i = 1:length(c)
		imwrite( reshape(Vc(i,:),[size(QQ,1),size(QQ,2)]), [name '_guess/Vc_' num2str(i) '+.png']);	
		imwrite( reshape(-Vc(i,:),[size(QQ,1),size(QQ,2)]), [name '_guess/Vc_' num2str(i) '-.png']);		
	end
	for i = 1:size(R1,1)
		imwrite( reshape(R1(i,:),[size(QQ,1),size(QQ,2)]), [name '_guess/R_recover_' num2str(i) '.png']);		
	end
	%if mod(t,10) == 0 
